...
public class BouteilleController {
    ...
    @PutMapping
    public ResponseEntity<?> misAJourBouteille(@Valid @RequestBody Bouteille bouteille) {

        try {
            if(bouteille == null || bouteille.getId() == null || bouteille.getId() <= 0){
                // Statut 204 : No Content - Pas de body car rien à afficher
                return ResponseEntity.status(HttpStatus.NOT_ACCEPTABLE).body("La bouteille et l'identifiant sont obligatoites");
            }
            bService.ajouter(bouteille);
            return ResponseEntity.ok(bouteille);
        } catch (RuntimeException e) {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> supprimerBouteille(@PathVariable("id") String idInPath) {
        try {
            final int id = Integer.parseInt(idInPath);
            bService.supprimer(id);
            return ResponseEntity.ok("Bouteille (" + id +") est supprime");
        } catch (NumberFormatException e) {
            return ResponseEntity.status(HttpStatus.NOT_ACCEPTABLE).body("Votre identifiant n'est pas un entier");
        } catch (RuntimeException e) {
